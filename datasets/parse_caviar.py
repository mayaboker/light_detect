...
def parse_data(root):
    #fnames = glob.glob(root + '/*.mpg')
    #for f in fnames:
    fname = Path(root) / 'Walk2.mpg'
    fname_gt = Path(root) / 'wk2gt.xml'
    cap = cv2.VideoCapture(fname.__str__())
    fps = cap.get(cv2.CAP_PROP_FPS)
    n_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    h = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    w = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    frames_path = Path(root) / 'Walk2'    
    ...
    for i, v in enumerate(data_dict):
        if v['objectlist'] is not None:
            ...
            if isinstance(obj, list):
                ...
                bbox = [xc-w//2, yc-h//2, xc+w//2, yc+h//2, id, orientation, appearance]
                img_name = str(frame_id).zfill(6) + '.png'
                gt[str(frame_id)] = {'bbox': bbox, 'img_name': img_name}
